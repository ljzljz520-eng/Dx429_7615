...
func (b *Builder) BuildIndex(job domain.CaptureJob, pages []domain.DocumentPage, manifest domain.BundleManifest) error {
	...
	tmpl := template.Must(template.New("index").Parse(`<!doctype html><html><head><meta charset="utf-8"><title>Offline bundle</title></head><body><h1>Offline bundle</h1><p>Status: {{.Job.Status}}</p><p>Incomplete: {{.Manifest.Incomplete}}</p><h2>Pages</h2><ul>{{range .Pages}}<li><a href="{{.Path}}">{{if .Title}}{{.Title}}{{else}}{{.URL}}{{end}}</a> ({{.Status}}){{if .Error}} - {{.Error}}{{end}}</li>{{end}}</ul>{{if .Manifest.FailedPages}}<h2>Failed pages</h2><ul>{{range .Manifest.FailedPages}}<li>{{.}}</li>{{end}}</ul>{{end}}{{if .Manifest.FailedAssets}}<h2>Failed assets</h2><ul>{{range .Manifest.FailedAssets}}<li>{{.}}</li>{{end}}</ul>{{end}}{{if .Manifest.ExternalLinks}}<h2>External links</h2><ul>{{range .Manifest.ExternalLinks}}<li><a href="{{.}}">{{.}}</a></li>{{end}}</ul>{{end}}</body></html>`))
